 ...
 * must implement {@link Comparable} interface. The values can be of
 * any type.
 *
 * <h3>Example</h3>
 *
...
public class FibonacciHeap<K extends Comparable<K>, V> {
     ...
     *
     * @complexity This operation can be done in O(1) but this implementation
     * takes O(<em>n</em>) in order to facilitate the garbage collection.
     */
    public void clear() {
        if (!isEmpty()) {
            min.clear();
     ...
     * @return a reference to the new node. Typically this reference is stored
     * and used in subsequent calls to {@link #decreaseKey(Node, Comparable)}
     * @complexity O(1)
     */
    ...
    /**
     * Merges two heaps. Warning: <b>this operation is destructive</b>. This
     * method empties the parameter {@code heap}.
     *
     ...
     *
     * @param x the node whose key is to be decreased. Must be a reference
     * returned by {@link #add(Comparable, Object)}.
     * @param key the new key
     * @throws IllegalArgumentException if the new key is greater than the
     * current.
